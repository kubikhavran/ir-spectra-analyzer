...
A few minutes later `IR-Spectra-Analyzer-Setup-0.27.0.exe` appears on the
...
IR_SPECTRA_VERSION=0.27.0 pyinstaller packaging/ir-spectra-analyzer-mac.spec --clean --noconfirm

# 4. Wrap in a DMG
dmgbuild -s packaging/dmg_settings.py \
  -D version=0.27.0 -D arch=arm64 \
  "IR Spectra Analyzer" \
  "dist/IR-Spectra-Analyzer-0.27.0-arm64.dmg"
```
